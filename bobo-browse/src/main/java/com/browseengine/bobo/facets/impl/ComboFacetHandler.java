...
public class ComboFacetHandler extends FacetHandler<FacetDataNone> {
  ...
  public RandomAccessFilter buildRandomAccessAndFilter(String[] vals, Properties prop)
    ...
    for (Entry<String, List<String>> entry : entries) {
      ...
      RandomAccessFilter f = facetHandler.buildRandomAccessAndFilter(
        selVals.toArray(new String[0]), prop);
  ...
  public FacetCountCollectorSource getFacetCountCollectorSource(BrowseSelection sel, FacetSpec fspec) {
    throw new UnsupportedOperationException("facet counting not supported for "
        + ComboFacetHandler.class);
  }
