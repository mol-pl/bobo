...
public class MultiValueFacetHandler extends FacetHandler<MultiValueFacetDataCache<?>> implements
    FacetScoreable {
  ...
  public MultiValueFacetHandler(String name, String indexFieldName,
      TermListFactory<?> termListFactory, Term sizePayloadTerm, Set<String> depends) {
    super(name, depends);
    _depends = depends;
  ...
  public MultiValueFacetHandler(String name, String indexFieldName,
      TermListFactory<?> termListFactory, Term sizePayloadTerm) {
  ...
  public RandomAccessFilter buildRandomAccessFilter(String value, Properties prop)
    ...
    AdaptiveFacetFilter af = new AdaptiveFacetFilter(new SimpleDataCacheBuilder(getName(),
        _indexFieldName), f, new String[] { value }, false);
    return af;
  }
  ...
  public static final class MultiValueDocScorer extends BoboDocScorer {
    ...
    public MultiValueDocScorer(MultiValueFacetDataCache<?> dataCache,
      ...
      super(scoreFunctionFactory.getFacetTermScoringFunction(dataCache.valArray.size(),
        dataCache._nestedArray.size()), boostList);
  ...
  public static final class MultiValueFacetCountCollector extends DefaultFacetCountCollector {
    ...
    MultiValueFacetCountCollector(String name, MultiValueFacetDataCache<?> dataCache, int docBase,
