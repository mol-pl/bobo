...
public final class BigNestedIntArray {
	...
	static public abstract class Loader {
		...
		/**
		 * loads data
		 * @throws java.lang.Exception Throw as needed.
		 */
	 ...
	 * @param size   .
	 * @param loader .
	 * @throws Exception loader failure.
	 */
	...
	public final boolean containsValueInRange(int id, int startValueId, int endValueId) {
		...
		if (val >= 0) {
			...
			while (idx < end) {
				...
				if (page[idx] >= startValueId && page[idx] < endValueId) {
					return true;
				}
				idx++;
			}
	...
	public final static class BufferedLoader extends Loader {
		 ...
		 * Resets loader.
		 *
		 * This also resets underlying BigIntBuffer.
		 *
		 * @param size new size.
		 * @param maxItems new max.
		 ...
		 * @param id  .
		 * @param val .
		 * @return false when exceeded the limit.
		 */
